...
export function ConversationsProvider({ id, children }) {
  ...
  // It is a useCallback function to prevent the making and destroying of socket event listeners
  // in the useEffect below during every rerender.
  ...
  // this use effect contains the event listener for 'recieve-message',
  // this is place in useEffect to prevent a new from being created
  // on every single re-render. We only want one.
  useEffect(() => {
  ...
  // user. It also calls the user's socket and passes the messages up to the server
  function sendMessage(recipients, text) {
    // emit message to server
    socket.emit("send-message", { recipients, text });
    // add message to local storage conversation using addMessageToConversation function
